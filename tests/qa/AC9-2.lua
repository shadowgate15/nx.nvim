local fzf_count = 0
package.loaded['fzf-lua'] = { fzf_exec = function() fzf_count = fzf_count + 1 end }
...
cache.get_project = function(ws, name, on_done)
  on_done({ ok = true, project = { targets = {} } })
end
...
vim.defer_fn(function()
  ...
  for _, n in ipairs(notify_calls) do
    if n.level == vim.log.levels.WARN and n.msg:find('alpha') then warned = true end
  end
  local ok = fzf_count == 0 and warned
  local line = ok and 'PASS: AC9-2' or ('FAIL: AC9-2 :: fzf_count=' .. fzf_count .. ' warned=' .. tostring(warned))
  vim.fn.writefile({ line }, '.sisyphus/evidence/qa-AC9-2.txt')
  vim.cmd('qa!')
end, 100)
